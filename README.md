...
このリポジトリには目的の違う2つのものが入っている。

| ディレクトリ | 内容 |
|---|---|
| ルート直下 (`prepare.py` / `train.py` / `program.md`) | AIが自律実験を繰り返した**スクリーニングモデルの実験コード**。以下の内容 |
| [`app/`](app/) | 日足を毎日取り込んで**指標で銘柄を検索するアプリ**（Web UI / API / CLI）。[app/README.md](app/README.md) |
